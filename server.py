...
async def magic_write(req: MagicWriteRequest):
    ...
    try:
        ...
        if is_low:
            prompt = (
                f"Write a {req.rating}-star Google review the way a let-down customer would actually type it on their phone.\n"
                f"Length: {length}. Tone: {tone}. {opener}\n"
                f"Who and where:\n"
                f"- {req.member_name} is the person who served them.\n"
                f"- It's a {req.category}.\n"
                f"- They left unhappy and want to raise: {themes}.\n"
                f"Make it sound like a real person:\n"
                f"- Stick to ONE or TWO real gripes — don't pile on every complaint at once.\n"
                f"- Ground each gripe in something you'd genuinely notice at a {req.category} "
                f"(for a dental clinic: teeth or gums that still didn't feel properly cleaned, plaque left behind; "
                f"for a restaurant: tables or plates that didn't look clean, food that seemed off).\n"
                f"- Pin it on whatever fits — the service, the visit, or {req.member_name} — however a customer would naturally say it.\n"
                f"- Use plain, everyday words and contractions (didn't, wasn't, I'd). A little casual or imperfect is fine.\n"
                f"- Disappointed and honest, not a furious rant. It must clearly read as a critical {req.rating}-star review, never glowing.\n"
                f"- No review-bot clichés or drama: avoid 'I am writing to express', 'utterly appalled', "
                f"'worst experience ever', 'do not recommend', 'unfortunately'. No quotation marks, emojis or hashtags.\n"
                f"Variation token (never mention it): {variation_seed}."
            )
            system_instruction = (
                f"You are a regular customer leaving an honest, disappointed Google review after a poor visit to a {req.category}. "
                f"Write like a real person texting it out — plain words, one or two specific complaints, fair rather than furious, "
                f"never an essay or a template. Tie what bothered you to real, concrete details from that kind of place. "
                f"Never use quotation marks."
            )
        else:
            prompt = (
                f"Write a {req.rating}-star Google review the way a happy customer would actually type it on their phone.\n"
                f"Length: {length}. Tone: {tone}. {opener}\n"
                f"Who and where:\n"
                f"- {req.member_name} is the person who helped them.\n"
                f"- It's a {req.category}.\n"
                f"- They left happy and want to call out: {themes}.\n"
                f"Make it sound like a real person:\n"
                f"- Pick just ONE or TWO concrete things to praise — real people don't list everything.\n"
                f"- Tie each one to something you'd genuinely notice at a {req.category} "
                f"(for a dental clinic: teeth feeling smooth and clean afterwards, a brighter smile; "
                f"for a restaurant: food tasting fresh, the place spotless).\n"
                f"- Credit it however a customer naturally would — the experience, the result, or {req.member_name}.\n"
                f"- Use plain, everyday words and contractions (I'd, they're, didn't). A little casual is fine.\n"
                f"- Mention {req.member_name} by name once, naturally — not in every sentence.\n"
                f"- No review-bot clichés: avoid 'highly recommend', 'top notch', 'went above and beyond', "
                f"'exceeded my expectations', 'a testament to', 'I recently had the pleasure'. No quotation marks, emojis or hashtags.\n"
                f"Variation token (never mention it): {variation_seed}."
            )
            system_instruction = (
                f"You are a regular customer dashing off a quick, genuine Google review after a good visit to a {req.category}. "
                f"Write the way people actually talk — short, specific, a touch casual, never marketing copy or an essay. "
                f"Focus on the one or two things you really noticed or felt. Skip stock phrases and never use quotation marks."
            )
